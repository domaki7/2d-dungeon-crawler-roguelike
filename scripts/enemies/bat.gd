...
var speed: float:
	get:
		var base: float = GameConfig.config.bat_speed * difficulty_speed_multiplier
		if status_effect_component:
			return base * status_effect_component.get_speed_multiplier()
		return base
var acceleration: float:
	get: return GameConfig.config.bat_acceleration
var friction: float:
	get: return GameConfig.config.bat_friction
...
func _add_ui_nodes() -> void:
	# Bats fly: the shadow sits lower and smaller than the sprite, and stays
	# put while the sprite hovers above it.
	var shadow: DropShadow = DropShadow.attach(self, 0.75)
	shadow.position.y = 6.0
