...
var death_delay: float:
	get: return GameConfig.config.archer_death_delay
...
func enter() -> void:
	...
	EventBus.enemy_killed.emit({"position": enemy.global_position, "type": "skeleton_archer"})
	...
	VFXHelper.spawn_death_decal(enemy.global_position, Color(0.62, 0.6, 0.52, 0.42))
