...
var death_delay: float:
	get: return GameConfig.config.ogre_death_delay
...
func enter() -> void:
	...
	EventBus.enemy_killed.emit({"position": enemy.global_position, "type": "ogre"})
	...
	VFXHelper.spawn_death_decal(enemy.global_position, Color(0.4, 0.12, 0.1, 0.5))
