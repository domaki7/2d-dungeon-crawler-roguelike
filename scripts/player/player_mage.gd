...
var acceleration: float:
	get: return GameConfig.config.mage_acceleration
var friction: float:
	get: return GameConfig.config.mage_friction
...
func _ready() -> void:
	...
	mana_component.mana_changed.connect(func(current: int, max_m: int) -> void:
...
func fire_bolt(direction: Vector2, damage_override: int = -1) -> void:
	...
	bolt.setup(direction, dmg)
	get_parent().add_child(bolt)
	AudioManager.play_sfx_varied(&"magic_bolt")

func fire_ice_shard(direction: Vector2, dmg: int, slow_effect: StatusEffectData) -> void:
	var shard: Area2D = ice_shard_scene.instantiate() as Area2D
	shard.global_position = global_position
	shard.setup(direction, dmg, slow_effect)
	get_parent().add_child(shard)
	AudioManager.play_sfx_varied(&"ice_shard")
