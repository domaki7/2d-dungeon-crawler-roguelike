...
## Blind Corners: freestanding L-shaped walls and short stubs create pockets
## you can't see into until you round the corner. Ambush-flavored layout.
...
func _paint_blind_walls() -> void:
	for y: int in range(3, 7):
		wall_layer.set_cell(Vector2i(7, y), 4, Vector2i(0, 0))
	for x: int in range(7, 11):
		wall_layer.set_cell(Vector2i(x, 3), 3, Vector2i(0, 0))
	for y: int in range(9, 13):
		wall_layer.set_cell(Vector2i(16, y), 4, Vector2i(0, 0))
	for x: int in range(13, 17):
		wall_layer.set_cell(Vector2i(x, 12), 3, Vector2i(0, 0))
	for x: int in range(2, 5):
		wall_layer.set_cell(Vector2i(x, 10), 3, Vector2i(0, 0))
	for x: int in range(19, 22):
		wall_layer.set_cell(Vector2i(x, 5), 3, Vector2i(0, 0))
