...
## The Crossways: four solid quadrant blocks leave only a cross of narrow
## corridors plus an outer ring. Fights funnel into chokepoints.
...
func _paint_quadrant_blocks() -> void:
	for x_start: int in [4, 14]:
		for y_start: int in [4, 9]:
			for x: int in range(x_start, x_start + 6):
				for y: int in range(y_start, y_start + 3):
					wall_layer.set_cell(Vector2i(x, y), 3, Vector2i(0, 0))
