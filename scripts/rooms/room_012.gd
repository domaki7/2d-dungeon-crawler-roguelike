...
## Twin Halls: a thick center wall splits the room into north and south halls
## connected only at the far left and right ends, forcing flanking runs.
...
func _paint_divider() -> void:
	for x: int in range(5, 19):
		wall_layer.set_cell(Vector2i(x, 7), 3, Vector2i(0, 0))
		wall_layer.set_cell(Vector2i(x, 8), 3, Vector2i(0, 0))
